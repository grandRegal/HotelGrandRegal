...
function RoomCatelog({ content, onView }) {
  const Features = ({ aminities }) => {
    return (
      <div className={roomCatelog.featureBox}>
        {
          aminities.slice(0, 6).map((feature) =>
  ...
  return (
    <div className={roomCatelog.container}>
      {
        content.map((roomInfoData) =>
          <div className={roomCatelog.roomBox} onClick={() => { onView(roomInfoData.id) }}>
            <img className={roomCatelog.thumbnail} src={roomInfoData.img} alt="" />
            <h2 className={roomCatelog.name}>{roomInfoData.name}</h2>
            <p className={roomCatelog.desc}>{roomInfoData.desc}</p>
            <Features aminities={roomInfoData.aminities} />
  ...
  return (
    <div className={roomInfo.container}>
      ...
      <div className={roomInfo.aminityBox}>
        {
          info.aminities.map((aminity) =>
            <span>
              <img src={aminity.logo} alt="" />
              {aminity.label}
            </span>
          )
        }
        ...
        <div className={roomInfo.ruleBox}>
          {
            info.rules.map((rule) =>
              <span>
                <img src={rule.logo} alt="" />
                {rule.label}
              </span>
            )
          }
        </div>
        <button onClick={() => { setPopup(<BookRoomStep1 brokerLink="https://a.com" onBookRequest={() => { setPopup(<BookRoomStep2 id={info.id} />) }} />) }}>Book Now</button>
      </div>
    </div>
  );
}
...
function BookRoomStep2(id) {
  ...
  const handleNext = ()=>{
    ...
    setPopup(<FinalReview formData = {formJSON} rate = {limitData.price} nightCount = {2} services= {formJSON.services}/>);
  }

  const handleCnfrmBook = (data, finalAmount)=>{
    ...
    const bookingConfirmation = bookRoom(data, finalAmount);
    ...
    <div style={{width:"280px", padding:"0px 20px", paddingBottom:"20px", marginTop:"-15px"}}>
      ...
      <p style={{textAlign:"center", color:"red"}}>Reason : {bookingConfirmation.error}</p>
    </div> : 
...
export default function Rooms() {
  ...
  const handleViewRoom = (roomIndex) => {
    let fetchedData = getRoomData(roomIndex);
    let roomInfoData = {
      id: roomIndex,
      thumbnail: roomList[roomIndex].img,
      name: roomList[roomIndex].name,
      overview: fetchedData.overview,
      gallery: fetchedData.gallery,
      aminities: roomList[roomIndex].aminities,
      rules: fetchedData.rules,
      price: roomList[roomIndex].price,
      broker: fetchedData.broker,
    };
  ...
  return (
    roomInfo ?
      ...
      <div>
        ...
        <RoomCatelog content={roomList} onView={handleViewRoom} />
      </div>
  );
}
